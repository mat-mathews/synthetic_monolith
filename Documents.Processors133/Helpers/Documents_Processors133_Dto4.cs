using Admin.Models199;
using Auth.Events;
using Auth.Mappers28;
using Billing.Core191;
using Billing.Shared312;
using Billing.Tests194;
using Common.Data126;
using DataAccess.Events283;
using Documents.Api132;
using Documents.Data490;
using GalaxyWorks.Mappers403;
using GalaxyWorks.Shared437;
using Imaging.Mappers;
using Reporting.Web345;
using Scheduling.Models441;
using Scheduling.Web221;
using Security.Mappers;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;
using Utilities.Data415;

namespace Documents.Processors133
{
    /// <summary>Immutable data transfer record for Documents_Processors133_Dto4.</summary>
    internal record Documents_Processors133_Dto4(string Value, int Count, DateTime Timestamp);

}