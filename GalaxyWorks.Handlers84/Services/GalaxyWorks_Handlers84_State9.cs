using Admin.Core121;
using Admin.Validators336;
using Billing.Client491;
using Common.Mappers;
using Common.Models;
using Common.Processors142;
using DataAccess.Events283;
using Documents.Api129;
using Export.Mappers;
using GalaxyWorks.Data;
using Import.Service15;
using Reporting.Service207;
using Scheduling.Handlers63;
using Scheduling.Processors80;
using Scheduling.Web19;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;
using Workflow.Shared;

namespace GalaxyWorks.Handlers84
{
    /// <summary>Defines the possible states for GalaxyWorks_Handlers84_State9.</summary>
    internal enum GalaxyWorks_Handlers84_State9
    {
        None = 0,
        Active = 1,
        Inactive = 2,
        Pending = 3,
        Processing = 4,
        Completed = 5,
        Failed = 6,
    }

}