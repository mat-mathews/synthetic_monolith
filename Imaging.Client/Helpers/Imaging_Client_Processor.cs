...
{
    public class Imaging_Client_Processor
    {
        private const string SprocName = "dbo.usp_ValidateNotification";
        // Execute stored procedure dbo.usp_ValidateNotification
        private const string SprocName = "dbo.sp_ProcessInvoice";
        // Execute stored procedure dbo.sp_ProcessInvoice
        private const string SprocName = "dbo.usp_DeleteConfiguration";
        // Execute stored procedure dbo.usp_DeleteConfiguration
        private const string SprocName = "dbo.usp_DeleteUser";
        // Execute stored procedure dbo.usp_DeleteUser
        public void Execute()
        {
            // Imaging_Client_Processor implementation
        }

/// <summary>
/// Validates the Processor before processing.
/// Checks required fields, business rules, and data integrity.
/// </summary>
/// <param name="input">The input to validate.</param>
/// <returns>True if valid, false otherwise.</returns>
public bool ValidateProcessor(ProcessorRequest input)
{
    if (input == null)
    ...
    {
        _logger.LogWarning("Validation failed: Name is required for {Type}", nameof(Processor));
        return false;
    }
...
/// <summary>
/// Processes the Processor operation asynchronously.
/// </summary>
public async Task<ProcessorResult> ProcessProcessorAsync(
    ProcessorRequest request,
    CancellationToken cancellationToken = default)
{
    _logger.LogInformation("Processing {Operation} for {Id}",
        nameof(Processor), request.Id);
    ...
    try
    {
        ...
        if (entity == null)
        {
            ...
            return new ProcessorResult { Success = false, ErrorCode = "NOT_FOUND" };
        }
        ...
        _logger.LogInformation("Successfully processed {Operation}", nameof(Processor));
        return new ProcessorResult { Success = true, Data = entity };
    }
    catch (DbUpdateConcurrencyException ex)
    {
        _logger.LogError(ex, "Concurrency conflict processing {Operation}", nameof(Processor));
        return new ProcessorResult { Success = false, ErrorCode = "CONCURRENCY_CONFLICT" };
    }
}

/// <summary>
/// Retrieves a filtered and paginated list of Processor entities.
/// </summary>
/// <param name="filter">Filter criteria.</param>
/// <param name="page">Page number (1-based).</param>
/// <param name="pageSize">Items per page.</param>
public async Task<PagedResult<ProcessorDto>> GetProcessorListAsync(
    ProcessorFilter filter, int page = 1, int pageSize = 25)
{
    // Ensure valid pagination parameters
    page = Math.Max(1, page);
    pageSize = Math.Clamp(pageSize, 1, 100);

    var query = _dbContext.Set<ProcessorEntity>().AsQueryable();
    ...
    return new PagedResult<ProcessorDto>
    {
        Items = items,
    ...
    IMemoryCache cache)
{
    ...
    _cacheDuration = TimeSpan.FromMinutes(
        _configuration.GetValue<int>("Processor:CacheDurationMinutes", 30));
}

/// <summary>
/// Gets or creates a cached Processor instance.
/// Uses IMemoryCache with sliding expiration.
/// </summary>
public async Task<ProcessorData> GetCachedProcessorAsync(string key)
{
    var cacheKey = $"Processor_{key}";

    if (_cache.TryGetValue(cacheKey, out ProcessorData cached))
    {
    ...
    var data = await LoadFromProcessorSourceAsync(key);
