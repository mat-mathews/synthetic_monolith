using Admin.Api;
using Auth.Handlers281;
using Auth.Handlers467;
using DataAccess.Validators;
using DataAccess.Web200;
using Documents.Client;
using Documents.Events451;
using Export.Api12;
using Export.Processors104;
using Imaging.Api;
using Imaging.Mappers93;
using Integration.Validators;
using Notifications.Tests;
using Portal.Service489;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;
using Utilities.Models41;
using Workflow.Data340;

namespace Scheduling.Mappers48
{
    internal interface IScheduling_Mappers48_Handler1
    {
        /// <summary>Processes the Scheduling_Mappers48_Handler1 operation.</summary>
        void ProcessScheduling_Mappers48_Handler1();

        /// <summary>Validates the Scheduling_Mappers48_Handler1 state.</summary>
        bool ValidateScheduling_Mappers48_Handler1();
    }

}