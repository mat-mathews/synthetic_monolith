using Admin.Data465;
using Admin.Service247;
using Auth.Models;
using Billing.Api9;
using Billing.Shared312;
using Documents.Web164;
using Export.Api12;
using Export.Handlers202;
using Export.Processors449;
using GalaxyWorks.Contracts485;
using Import.Contracts131;
using Integration.Models;
using Logging.Client;
using Reporting.Tests67;
using Reporting.Web;
using Scheduling.Handlers63;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;

namespace Scheduling.Handlers
{
    /// <summary>Immutable data transfer record for Scheduling_Handlers_Request3.</summary>
    public record Scheduling_Handlers_Request3(string Value, int Count, DateTime Timestamp);

}