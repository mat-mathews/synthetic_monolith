using Admin.Data465;
using Admin.Mappers;
using Admin.Web;
using BatchJobs.Api;
using Common.Data21;
using Export.Client13;
using Export.Events;
using Export.Handlers;
using Export.Models;
using Imaging.Web172;
using Import.Events374;
using Notifications.Models;
using Notifications.Web90;
using Reporting.Events188;
using Scheduling.Tests;
using Security.Api134;
using Security.Validators428;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;
using Utilities.Shared114;

namespace GalaxyWorks.Validators355
{
    /// <summary>Defines the possible states for GalaxyWorks_Validators355_Status3.</summary>
    internal enum GalaxyWorks_Validators355_Status3
    {
        None = 0,
        Active = 1,
        Inactive = 2,
        Pending = 3,
        Processing = 4,
        Completed = 5,
        Failed = 6,
    }

}